...
object RCPornographicDiscriminationPrediction extends TaskSchedule{
  def main(args:Array[String]){
    ...
    {
      var res:Int = -1
      val rate = (violations_count  + sexual_count * 0.5) / (violations_count+sexual_count+normal_count)
      if(rate>0.5){
        res = 1
      }else{
        res = 0
    ...
    // 完整特征合并
    val resboyDF = boyProfileDF
      .join(boyToGirlProfileDF, Seq("user_id"), "left_outer")
    val resgirlDF = girlProfileDF
      .join(boyToGirlProfileDF, Seq("target_user_id"), "left_outer")
    val resDF = resboyDF.union(resgirlDF)

    // 特征保存
//    deleteFile(spark, "/user/spark/recommend/LogisticRegression/features/boy/", offsetDay, historyOffsetDay)
    boyProfileDF.write.save("/user/spark/recommend/LogisticRegression/features/boy/" + offsetDay)
//    deleteFile(spark, "/user/spark/recommend/LogisticRegression/features/girl/", offsetDay, historyOffsetDay)
    girlProfileDF.write.save("/user/spark/recommend/LogisticRegression/features/girl/" + offsetDay)
//    deleteFile(spark, "/user/spark/recommend/LogisticRegression/features/boyToGirl/", offsetDay, historyOffsetDay)
    boyToGirlProfileDF.write.save("/user/spark/recommend/LogisticRegression/features/boyToGirl/" + offsetDay)
